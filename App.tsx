...
import { 
  ...
  Bell, Menu, X, Search, BookOpen, UserCheck, LayoutDashboard, Search as SearchIcon, Sparkles
} from 'lucide-react';
...
// This component contains the logic that depends on the Router context
const AppShell: React.FC = () => {
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden text-slate-900 font-['Inter']">
      ...
      <aside className={`
        fixed lg:relative z-50 h-full bg-white border-r border-slate-200 transition-all duration-300 ease-in-out flex flex-col shadow-xl lg:shadow-none
      ...
      `}>
        ...
        <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
          ...
          {currentUser.role === 'STUDENT' && (
            <>
              <div className={`pt-6 pb-2 px-3 ${!isSidebarOpen && 'hidden'}`}>
          ...
          {currentUser.role === 'REVIEWER' && (
            <NavLink to="/review-queue" icon={<ShieldCheck size={18} />} label="Queue" isOpen={isSidebarOpen} />
          )}

          {currentUser.role === 'ADMIN' && (
            <NavLink to="/admin-approval" icon={<UserCheck size={18} />} label="Admin" isOpen={isSidebarOpen} />
          )}
        </nav>
        ...
        <div className="p-3 border-t border-slate-100 space-y-2">
          ...
          <button 
            ...
            className={`w-full flex items-center gap-3 px-3 py-2.5 text-rose-500 hover:bg-rose-50 rounded-xl transition-all text-sm font-semibold ${!isSidebarOpen && 'justify-center'}`}
          >
...
const NavLink = ({ to, icon, label, isOpen }: any) => {
  ...
  return (
    <Link
      ...
      `}
    >
      <div className={`shrink-0 ${isActive ? 'text-white' : 'text-slate-400 group-hover:text-indigo-600'}`}>
        {icon}
      </div>
      <span className={`text-xs font-bold tracking-wide truncate transition-all duration-300 ${isOpen ? 'opacity-100' : 'opacity-0 w-0'}`}>
        {label}
      </span>
    </Link>
  );
...
// The root App component provides the Router context
const App: React.FC = () => {
  return (
    <Router>
      <AppShell />
